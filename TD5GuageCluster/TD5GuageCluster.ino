...
vcc-3.3v
*/
//cs pins sending to the screens
#define CS_psi 5
#define CS_temp 0
#define CS_erg 4
...
int psiVal = 0;
//gpio pin reading from the sensor
const int GPIO_PSI = 12; 
...
void setup() {
  ...
  //set the pin mode of the cs pins for the displays
  ...
  //now that tft object has been inialise for all screens set the cs pins to high
  digitalWrite(CS_psi, HIGH);
  digitalWrite(CS_temp, HIGH);
  digitalWrite(CS_erg, HIGH);
...
void RunSplashScreen() {
  ...
  //we only need one dialface object, its is reused and sent to all screens
...
void ConfigureDials() {
  ...
  // delete the previous spite used for the splash screen
  dialFace.deleteSprite();

  //configure the dial face object
  dialFace.setColorDepth(8);
  dialFace.setSwapBytes(true);
  dialFace.createSprite(240, 240);
...
void SetupPSITickArray(float refVal) {
  ...
  //draw 100 markers on 0 to 100 dial face
  for (int i = 0; i < 101; i++) {
...
void SetupTEMPTickArray(float refVal) {
  ...
  float tickRef = refVal;
  //draw 120 markers on a 0 to 120 dial face
  for (int i = 0; i < 121; i++) {
    // the angle is always radians as thats what cos & sine need so conver degrees to radians with rad
